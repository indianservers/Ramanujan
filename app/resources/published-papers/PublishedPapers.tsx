...
export function PublishedPapers() {
  ...
  return (
    <section>
      ...
      {activeFilters.length ? <div className="active-filter-chips paper-active-filters">{activeFilters.map((item) => <button key={item.label} type="button" onClick={item.clear}>{item.label}: <span>{item.value}</span> x</button>)}<button type="button" onClick={clearFilters}>Clear all</button></div> : null}
      ...
      <div className="formula-grid">
        {visible.map((paper) => {
          ...
          return <article className="formula-card paper-card" key={paper.slug}><p className="detail-meta">{paper.year} | {paper.journal}</p><h2>{paper.title}</h2><p className="paper-biblio">{paper.volumePages ?? "Bibliographic details incomplete."}</p><div className="formula-card-tags">{paper.subjects.map((item) => <span key={item}>{item}</span>)}</div><p>{paper.collaborators.length ? `With ${paper.collaborators.join(", ")}` : "Solo paper by Srinivasa Ramanujan"}</p>{related.length ? <div className="paper-related-links"><strong>Related discoveries</strong>{related.map((item) => item ? <a key={item.slug} href={`/discoveries/${item.slug}`}>{item.title}</a> : null)}</div> : null}<p className="citation">{paper.externalSource ?? "Collected-paper reference"} | {paper.reviewStatus}</p><SaveButton id={`paper:${paper.slug}`} label={paper.title} /></article>;
        })}
      </div>
    </section>
  );
